...
use moonsock::{
    JsonRpcNotification,
    MoonNotification, NotificationMethod, NotificationParam,
    jsonrpc_ws_client::JsonRpcVersion,
};
...
fn test_moon_notification_try_from_unknown_method() {
    ...
    assert!(result.unwrap_err().to_string().contains("Error parsing notification method"));
}
